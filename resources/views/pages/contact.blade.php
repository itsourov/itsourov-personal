                    ...
                    <form action="#" method="post">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-input.text name="name" :value="old('name')" type="text" class="mt-1 block w-full"
                                    autocomplete="name" />
                                <x-input.error :messages="$errors->get('name')" />
                            </div>
                            <div>
                                <x-input.label :value="__('Email address')" required="true" />
                                <x-input.text name="email" :value="old('email')" type="text" class="mt-1 block w-full"
                                    autocomplete="email" />
                                <x-input.error :messages="$errors->get('email')" />
                            </div>
                            <div>
                                <x-input.label :value="__('Phone number')" required="true" />
                                <x-input.text name="phone" :value="old('phone')" type="text" class="mt-1 block w-full"
                                    autocomplete="phone" />
                                <x-input.error :messages="$errors->get('phone')" />
                            </div>
                            <div>
                                <x-input.label :value="__('Company name') . ' (' . __('if available') . ')'" />
                                <x-input.text name="company" :value="old('company')" type="text" class="mt-1 block w-full"
                                    autocomplete="company" />
                                <x-input.error :messages="$errors->get('company')" />
                            </div>
                            <div class=" col-span-2">
                                <x-input.label :value="__('Message')" required="true" />
                                <x-input.textarea name="message" type="text" class="mt-1 block w-full"
                                    rows="4">
                                    {{ old('message') }}</x-input.textarea>
                                <x-input.error :messages="$errors->get('message')" />
                            </div>
                        </div>
                        <x-button.primary type="button" class="w-full mt-4 py-3">{{ __('Send') }}</x-button.primary>
                    </form>
                </x-card>
            </div>
        </div>
