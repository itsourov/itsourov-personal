<div>
    <button class="replyButton text-sm hover:text-primary-500" x-on:click="replyBoxOpen = !replyBoxOpen">
        <i class="fa-solid fa-comment-dots"></i>
        {{ __('Relpy') }}
    </button>
</div>
<div class="space-y-3" x-transition x-show="replyBoxOpen" x-cloak>
    <x-input.textarea wire:model.lazy="replyBoxTexts.{{ $comment->id }}" class="replyBoxTexts text-sm"
        placeholder="{{ __('Write a reply') }}...">

    </x-input.textarea>
    <x-button.primary class="text-xs" wire:click="addReply('{{ $comment->id }}')">
        <div wire:loading wire:target="addReply">
        ...
        <div wire:loading.remove wire:target="addReply">
            {{ __('Post Reply') }}
        </div>
